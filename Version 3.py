...
# The file where all category and task data will be saved
DATA_FILE = "todo_data.json"
 
# Global list storing all categories
# Each item: {"name": str, "tasks": [{"name": str, "completed": bool}]}
all_categories = []
...
def load_data():
    # Load saved categories from the JSON file when the program starts.
...
def save_data():
    # Save all current categories and tasks to the JSON file.
    with open(DATA_FILE, "w") as file:
        json.dump(all_categories, file, indent=4)
 
 
def on_close():
    # Run when the user closes the window — saves data then closes.
    save_data()
    window.destroy()
...
def get_selected_task_index():
    # Return the index of the currently selected task, or None if nothing selected.
...
def remove_task():
    # Remove all selected tasks from the currently selected category.
    # Supports removing a single task or multiple tasks at once.
    if current_category_index is None:
        messagebox.showwarning("No category selected", "Please select a category first.")
        return
 
    # curselection() returns a tuple of every highlighted index,
    # e.g. (0, 2, 3) if the user ctrl-clicked multiple tasks
    selected_indexes = task_box.curselection()
    if not selected_indexes:
        messagebox.showwarning("No task selected", "Please select at least one task first.")
        return
 
    # Build a list of the task names being removed, for the confirmation message
    task_names = [all_categories[current_category_index]["tasks"][i]["name"] for i in selected_indexes]
 
    if len(task_names) == 1:
        message = f"Remove task '{task_names[0]}'?"
    else:
        message = f"Remove {len(task_names)} tasks?\n\n" + "\n".join(task_names)
 
    confirm = messagebox.askyesno("Remove Task", message)
    if confirm:
        # Remove from highest index to lowest so removing one task doesn't
        # shift the positions of the remaining tasks still to be removed
        for index in sorted(selected_indexes, reverse=True):
            all_categories[current_category_index]["tasks"].pop(index)
        refresh_task_box()
...
# Tell tkinter to run on_close (which saves data) when the user clicks the X button,
# instead of just closing immediately and losing unsaved changes
...
# selectmode=tk.EXTENDED allows ctrl/shift-click to select multiple tasks
task_box = tk.Listbox(right_frame, width=35, height=15, selectmode=tk.EXTENDED)
task_box.pack()
 
tk.Button(right_frame, text="+ Add Task", width=25, command=add_task).pack(pady=2)
tk.Button(right_frame, text="- Remove Task(s)", width=25, command=remove_task).pack(pady=2)
tk.Button(right_frame, text="✔ Mark Complete", width=25, command=mark_complete).pack(pady=2)
 
# Load saved data and populate the category box before starting the window
load_data()
refresh_category_box()
